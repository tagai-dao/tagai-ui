...
- 新 Service Worker 下载完成后保持等待，不主动接管正在打开的页面，也不强制整页刷新。关闭本站所有标签页及 PWA 窗口后重新打开，启用已下载的新版本；仅按刷新键不保证激活等待中的版本。
- 发布后验收新功能时，请先完成上述关闭并重开，避免仍在测试旧版本。页面和缓存资源在当前会话中保持同一版本。
- LP 页面连接、切换或断开钱包时，保留卡片、展开状态、输入和滑点，只更新数据。新账户数据返回前，个人余额/奖励显示占位符并禁止提交；定时刷新不清空表单。
- 切换代币、矿池或产品链仍会重置对应数据，避免串用其他池子或账户的数据。钱包主动断开、浏览器回收页面不属于数据刷新的保证范围。
