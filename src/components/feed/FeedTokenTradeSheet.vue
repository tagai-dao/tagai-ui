      ...
      <div v-if="modelValue && asset" class="feed-trade-sheet-layer fixed inset-x-0 top-0 z-[140]">
        ...
        >
          ...
          <div ref="scroller" class="min-h-0 flex-1 overflow-y-auto px-2 pb-[calc(12px+env(safe-area-inset-bottom))] pt-2 no-scroll-bar">
            <BuyAndSellView :key="`${asset.tick}-${asset.tweetId || ''}-${asset.commerceId || ''}-${asset.sellsman || ''}`" :tweet-id="asset.tweetId" :tick="asset.tick" :sellsman="asset.sellsman" :commerce-id="asset.commerceId" :login-return-path="asset.loginReturnPath" />
          </div>
        </section>
      </div>
    </Transition>
