...
const {formatEmojiText, preCheckCuration, userTip} = useTweet()
const {
  contentEl,
  contentRef,
  getBlur,
...
const preTip = async () => {
  try{
    ...
    if (await preCheckCuration(OperateType.TIP, props.tweet, costVp.value)) {
...
const tip = async () => {
  try{
    isQuoting.value = true;
    await userTip(props.tweet, props.tweet.tick!, costVp.value)
    quoteVisible.value = false
  } catch (e) {
    handleErrorTip(e)
  } finally {
    isQuoting.value = false
    props.tweet.tipped = 1;
  }
}
