...
<!--        </button>-->
            ...
            <div v-if="kolFee > 0" class="flex items-center justify-between bg-gray-50 rounded-full px-4 h-10 mt-3">
              <div class="flex gap-2 items-center">
                ...
                <el-tooltip popper-class="c-arrow-popper">
                  <template #content>
                    <div class="text-white p-2 max-w-200px text-xs">{{ $t('profileView.feeIncomeDesc', { symbol: nativeSymbol }) || `Users buy/sell IPShare pay fees in ${nativeSymbol}. 4.5% goes to the KOL and 2.5% to the protocol.` }}</div>
                  </template>
                  <button>
    ...
    <div ref="profileTabs" class="profile-content-tabs" role="tablist" aria-label="Profile content">
      <button v-for="tab of tabOptions" :key="tab"
              role="tab" :aria-selected="tab === activeTab"
              @click="activeTab=tab">{{$t('profileView.'+tab)}}</button>
    </div>
    <div ref="profileContent" class="profile-scroll-content">
