...
const bidCommunityPath = computed(() => getChainPath(deployment.value.chainId, `/tag-detail/${deployment.value.chainId === 56 ? 'BUIDL' : 'TagAgent'}`))
    ...
    <template v-else>
      ...
      <section class="panel auction-panel"><div class="panel-title"><div><span>BUYBACK</span><h2>{{ deployment.chainId === 56 ? $t('baskets.buidlBuybackPool') : $t('baskets.buybackAuction') }}</h2></div><em>{{ loadingFees ? $t('baskets.loading') : $t('baskets.liveOnChain') }}</em></div>
        ...
        <div v-if="activeAuction" class="active-auction">
          ...
          <div class="auction-values"><div><span>{{ $t('baskets.ethLot').replace('ETH', deployment.nativeSymbol) }}</span><strong>{{ formatToken(activeAuction.ethAmount) }} {{ deployment.nativeSymbol }}</strong></div><div><span>{{ $t('baskets.highestBid') }}</span><strong>{{ formatToken(activeAuction.highestBid, bidDecimals) }} <RouterLink class="bid-token-link" :to="bidCommunityPath">{{ bidSymbol }}</RouterLink></strong></div><div><span>{{ $t('baskets.highestBidder') }}</span><strong>{{ short(activeAuction.highestBidder) }}</strong></div></div>
        ...
        <div v-else class="create-auction"><div><h3>{{ $t('baskets.startAuction') }}</h3><p class="auction-threshold">{{ $t('baskets.auctionThresholdHint', { threshold: formatToken(minAuctionEth), native: deployment.nativeSymbol }) }}</p><i18n-t keypath="baskets.startAuctionHint" tag="p" scope="global"><template #amount>{{ formatToken(openingBid, bidDecimals) }}</template><template #symbol><RouterLink class="bid-token-link" :to="bidCommunityPath">{{ bidSymbol }}</RouterLink></template></i18n-t></div><button class="primary" :disabled="!account || !canCreateAuction || !spotQuote || !!action" @click="createAuction">{{ action === 'approve' ? $t('baskets.approving') : action === 'create' ? $t('baskets.startingAuction') : $t('baskets.startAuction') }}</button></div>
        <div v-if="account" class="account-balances"><div><span>{{ $t('baskets.bidTokenBalance') }}</span><strong>{{ formatToken(bidBalance, bidDecimals) }} <RouterLink class="bid-token-link" :to="bidCommunityPath">{{ bidSymbol }}</RouterLink></strong></div><div><span>{{ $t('baskets.refundableBid') }}</span><strong>{{ formatToken(availableBidTokens, bidDecimals) }} <RouterLink class="bid-token-link" :to="bidCommunityPath">{{ bidSymbol }}</RouterLink></strong></div><div><span>{{ $t('baskets.claimableAuctionEth').replace('ETH', deployment.nativeSymbol) }}</span><strong>{{ formatToken(claimableEth) }} {{ deployment.nativeSymbol }}</strong></div></div>
      ...
      <section class="panel"><div class="panel-title"><div><span>HISTORY</span><h2>{{ $t('baskets.auctionHistory') }}</h2></div></div>
        ...
        <div v-else class="history"><article v-for="item in auctions" :key="item.id.toString()"><b>#{{ item.id }}</b><div><span>{{ $t('baskets.ethLot').replace('ETH', deployment.nativeSymbol) }}</span><strong>{{ formatToken(item.ethAmount) }} {{ deployment.nativeSymbol }}</strong></div><div><span>{{ $t('baskets.winningBid') }}</span><strong>{{ formatToken(item.highestBid, bidDecimals) }} <RouterLink class="bid-token-link" :to="bidCommunityPath">{{ bidSymbol }}</RouterLink></strong></div><div><span>{{ $t('baskets.winner') }}</span><strong>{{ short(item.highestBidder) }}</strong></div><em :class="{ done: item.settled }">{{ item.settled ? $t('baskets.settled') : chainNow !== undefined && item.endTime * 1000 <= chainNow ? $t('baskets.awaitingSettlement') : $t('baskets.active') }}</em></article></div>
      </section>
    </template>
  </div></div>
</template>
...
<style scoped>
.bid-token-link { color: var(--auction-link-color, #7250cf); text-decoration: underline; text-decoration-color: currentColor; text-underline-offset: 3px; transition: color .15s; }
.bid-token-link:hover { color: var(--auction-link-hover, #5532b1); }
:global(html.dark .bid-token-link) { --auction-link-color: #b39aff; --auction-link-hover: #d0bfff; }
.bid-token-link:focus-visible { outline: 2px solid #9c88ff; outline-offset: 3px; border-radius: 3px; }
