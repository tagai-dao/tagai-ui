...
<template>
  <div>
    <div v-if="comStore.currentSelectedCommunity?.tick"
      ...
      <iframe v-else :src="`https://dexscreener.com/bsc/${(comStore.currentSelectedCommunity?.version === 7 || comStore.currentSelectedCommunity?.version === 8) ? comStore.currentSelectedCommunity?.token : comStore.currentSelectedCommunity?.pair}?embed=1&loadChartSettings=0&trades=0&tabs=0&chartLeftToolbar=0&chartTimeframesToolbar=0&info=1&loadChartSettings=0&chartDefaultOnMobile=1&chartTheme=light&theme=light&chartStyle=1&chartType=usd&interval=15`" 
