...
export const useStateStore = defineStore('state', {
  state (){
    return {
      ...
      // Coin 菜单的子标签页：自选、标签币、资产篮、bStocks
      coinSubMenu: 'tagCoin' as 'watchlist' | 'tagCoin' | 'baskets' | 'bStocks' | 'ip',
      // 兼容旧代码，保持 activeHomeTab
      activeHomeTab: 'tagCoin' as 'tweets' | 'prediction' | 'watchlist' | 'tagCoin' | 'baskets' | 'bStocks' | 'ip' | 'mindshare',
  ...
  actions: {
    ...
    // 设置 Coin 子菜单
    setCoinSubMenu(subMenu: 'watchlist' | 'tagCoin' | 'baskets' | 'bStocks' | 'ip') {
      this.coinSubMenu = subMenu
      this.activeHomeTab = subMenu
      this.activeMainMenu = 'coin'
    },
    // 兼容旧代码的方法
    setActiveHomeTab(tab: 'tweets' | 'prediction' | 'watchlist' | 'tagCoin' | 'baskets' | 'bStocks' | 'ip') {
      this.activeHomeTab = tab
      if (tab === 'tweets' || tab === 'prediction') {
        this.tagSubMenu = tab
        this.activeMainMenu = 'tag'
        this.homeSubMenu = tab
      } else if (tab === 'watchlist' || tab === 'tagCoin' || tab === 'baskets' || tab === 'bStocks' || tab === 'ip') {
