...
const {poolAprBps,readPoolRewards}=createRequire(import.meta.url)(join(dir,'apr.cjs'))
...
test('APR values both sides of staked LP, not all LP in circulation',()=>{
 ...
 assert.equal(poolAprBps(1n,1000n,100n,10n,true),18250n)
 assert.equal(poolAprBps(1n,1000n,100n,20n,true),9125n)
})
...
test('large token supplies use integer arithmetic with no loss from JS numbers',()=>{
 const scale=10n**30n
 assert.equal(poolAprBps(scale,1000n*scale,100n*scale,10n*scale,true),18250n)
})
function fixture(ratio=8000n){
 const requests=[]
 globalThis.__aprClient={getBlock:async()=>({number:123n,timestamp:36123n}),getStorageAt:async p=>{requests.push(p);return '0x'+ratio.toString(16).padStart(64,'0')},readContract:async p=>{requests.push(p);return p.functionName==='feeRatio'?1000:p.functionName==='rewardCalculator'?calculator:1000n}}
 return requests
}
...
test('reads 24 hourly buckets at the chain clock, subtracts community fee and applies live pool ratio',async()=>{
 ...
 assert.deepEqual(r,{daily:720n,ratio:8000,block:123n})
 assert.ok(requests.every(r=>r.blockNumber===123n))
 assert.deepEqual(requests.find(r=>r.functionName==='calculateReward').args,[community,36000n,122400n])
})
