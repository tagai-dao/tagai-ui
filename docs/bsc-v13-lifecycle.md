...
- 创建成功后先保存交易登记任务，API 上传失败在后台静默重试，不要求用户再次创建；本地测试同样保留 Logo 必填校验，测试图片由管理面板提供下载，走原上传流程。
- LP 矿池使用翻转卡片，显示成分 Logo、权重、APR、总质押和我的质押；双资产输入可相互推算，余额及 LP 预估保持紧凑展示。内盘 / pendinglist 顶部显示销售进度，只有已上市才显示销毁总量并读取 T 的销毁地址余额。
- 生命周期与 API 详情独立加载。矿池详情 API 暂不可用时，按同一区块读取 Token 成分和 Community 的 `createdPools`，校验社区代币、矿池 `stakeToken` / `community` 与成分 Pair 一致后显示卡片，并提示使用链上核验数据；不伪造指数 / 回购元数据，也不补写数据库。兜底失败仍报错，但不丢弃已读到的销售进度。回归：`node --test scripts/test-v13-mining-panel.mjs`。
